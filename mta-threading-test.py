...
def print_feed(feed, line_id, local_lines_array, local_stops_array):
    for trip in feed.filter_trips(line_id):
        if trip.location in brokenstops:
            continue
        stop_check = str(trip)
        if "STOPPED_AT" in stop_check:
            stop_ind = stop_check.find('STOPPED_AT') + len('STOPPED_AT ')
            last_ind = stop_check.find(', last update')
            local_lines_array.append(stop_check[11:12])
            local_stops_array.append(stop_check[stop_ind:last_ind])

def fetch_line_feed(line_num):
    local_lines_array = []
    local_stops_array = []
    line_id = process_lines(line_num)
    print(f"Reading MTA Feed for the {line_id} Trains...")
    feed = NYCTFeed(line_id, api_key=MTA_API_KEY)
    print_feed(feed, [line_id], local_lines_array, local_stops_array)

    # Batch update shared lists
    with lock:
        lines_array.extend(local_lines_array)
        stops_array.extend(local_stops_array)

def main():
    with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust max_workers as needed
        futures = [executor.submit(fetch_line_feed, line_num) for line_num in lines]

        for future in as_completed(futures):
            try:
                future.result()  # To handle any exceptions that may occur
...
if __name__ == '__main__':
    ...
    for i in range(len(lines_array)):
        combined_array.append(f"{lines_array[i]} {stops_array[i]}")
